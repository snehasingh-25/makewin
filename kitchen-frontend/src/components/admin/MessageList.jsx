...
export default function MessageList({ messages, onUpdate }) {
  ...
  const columns = [
      ...
      render: (m) => (
        <div>
          <div className="font-semibold text-gray-900 flex items-center gap-2">
            {m.name}
            {!m.read && (
              <span className="inline-block px-2 py-0.5 text-xs bg-olive text-white rounded-full font-semibold">
                New
              </span>
            )}
          </div>
    ...
    },
    {
      ...
      render: (m) => (
        <span
          className={`inline-block px-2 py-1 text-xs rounded-full font-semibold ${m.read ? "bg-gray-100 text-gray-700" : "bg-olive/10 text-olive"
            }`}
        >
          {m.read ? "Read" : "New"}
        </span>
        ...
        <div className="flex gap-2">
          {!message.read && (
            <button
              ...
              className="px-3 py-1.5 bg-olive text-white rounded-lg text-sm font-semibold hover:bg-olive-dark transition"
            >
              Mark Read
            </button>
          )}
